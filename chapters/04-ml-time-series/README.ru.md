...
Машинное обучение (ML) радикально изменило подход к анализу финансовых рынков. Однако применение ML в трейдинге существенно отличается от классических задач — здесь данные нестационарны, сигнал слаб относительно шума, а цена ошибки измеряется реальными деньгами.

В этой главе мы построим production-ready систему машинного обучения для финансовых временных рядов, используя Rust для высокопроизводительного feature engineering и inference.

---

## 4.1 Особенности ML для финансовых данных

### 4.1.1 Ключевые проблемы

| Проблема | Описание | Решение |
|----------|----------|---------|
| Нестационарность | Распределение данных меняется со временем | Walk-forward валидация, детекция режимов |
| Низкий SNR | Большинство движений — шум | Тщательный feature engineering, ансамбли |
| Lookahead bias | Использование будущей информации при обучении | Строгое разделение по времени |
| Survivorship bias | Данные только "выживших" активов | Включение делистингованных бумаг |
| Data snooping | Множественное тестирование гипотез | Out-of-sample валидация, поправка Бонферрони |
| Смена режимов | Разные рыночные условия | Regime-aware модели |

### 4.1.2 Walk-Forward валидация

Классическое разделение train/test случайным образом **недопустимо** для временных рядов:

```rust
/// Walk-forward валидация с embargo периодом
/// Предотвращает lookahead bias через временной зазор между train и test
...
impl WalkForwardValidator {
    ...
    /// Генерирует индексы для train/test разбиений
...
mod tests {
    ...
    fn test_walk_forward_splits() {
        ...
        // Проверяем, что test всегда после train + embargo
...
Метод маркировки от Маркоса Лопеса де Прадо — вместо простого "вверх/вниз" учитываем три барьера:

```rust
/// Результат маркировки по трём барьерам
#[derive(Debug, Clone, Copy, PartialEq)]
pub enum TripleBarrierLabel {
    /// Цена достигла верхнего барьера (profit target)
    TakeProfit = 1,
    /// Цена достигла нижнего барьера (stop loss)
    StopLoss = -1,
    /// Истёк горизонт времени без пробития барьеров
    TimeOut = 0,
}

/// Triple Barrier Labeling для создания ML таргетов
pub struct TripleBarrierLabeler {
    /// Порог для take profit (например, 0.02 = 2%)
    upper_barrier: f64,
    /// Порог для stop loss (например, -0.02 = -2%)
    lower_barrier: f64,
    /// Максимальный горизонт в периодах
    max_horizon: usize,
}
...
impl TripleBarrierLabeler {
    pub fn new(upper_barrier: f64, lower_barrier: f64, max_horizon: usize) -> Self {
        Self {
            upper_barrier,
            lower_barrier: lower_barrier.abs() * -1.0, // Гарантируем отрицательное значение
            max_horizon,
        }
    }

    /// Маркирует одну точку данных
    pub fn label_single(&self, prices: &[f64], start_idx: usize) -> Option<TripleBarrierLabel> {
        ...
        for i in 1..=self.max_horizon {
            ...
            // Записываем первое касание каждого барьера
        ...
        // Определяем, какой барьер сработал первым
...
- **Скорость**: Real-time вычисления для live trading
- **Память**: Zero-copy операции для больших datasets
- **Надёжность**: Компилятор предотвращает race conditions
- **Интеграция**: Легко встраивается в streaming pipeline

### 4.2.2 Технические индикаторы

```rust
use std::collections::VecDeque;

/// Высокопроизводительный калькулятор технических индикаторов
/// Использует ring buffer для streaming вычислений
pub struct TechnicalIndicators {
    /// Буфер для скользящих вычислений
...
impl TechnicalIndicators {
    ...
    /// Bollinger Bands: возвращает (lower, middle, upper)
    ...
    pub fn macd(
        ...
        // Для полного MACD нужна история macd_line
        // Упрощённая версия: signal = EMA(macd_line)
        // В production используйте отдельный буфер для macd_line
        let signal_line = macd_line * 0.9; // Упрощение
...
### 4.2.3 Микроструктурные features

```rust
/// Features из микроструктуры рынка (order book, trades)
pub struct MicrostructureFeatures;

impl MicrostructureFeatures {
    /// Order Book Imbalance
    /// Показывает дисбаланс между bid и ask объёмами
    /// Значения от -1 (преобладает ask) до +1 (преобладает bid)
    ...
    /// Microprice
    /// Более точная оценка "справедливой" цены, чем mid price
    /// Взвешивает цены по противоположным объёмам
    pub fn microprice(bid: f64, ask: f64, bid_volume: f64, ask_volume: f64) -> f64 {
        let total_volume = bid_volume + ask_volume;
        if total_volume == 0.0 {
            return (bid + ask) / 2.0;
        }
        // Большой ask_volume толкает цену к bid, и наоборот
        (bid * ask_volume + ask * bid_volume) / total_volume
    }

    /// Spread в базисных пунктах
    ...
    /// VWAP Deviation
    /// Отклонение текущей цены от VWAP
    ...
    /// Kyle's Lambda — оценка price impact
    /// Линейная регрессия: returns ~ signed_volume
    ...
    /// Trade Flow Imbalance за период
    /// Отношение buy volume к total volume
...
## 4.3 Архитектуры глубокого обучения

### 4.3.1 LSTM для временных рядов

LSTM (Long Short-Term Memory) решает проблему исчезающего градиента и способен запоминать долгосрочные зависимости.

```rust
/// Структура для хранения параметров LSTM слоя
/// В production используйте готовые библиотеки (tch-rs, burn)
/// Это — образовательная реализация forward pass
pub struct LSTMCell {
    /// Размер входа
    input_size: usize,
    /// Размер скрытого состояния
    hidden_size: usize,
    /// Веса для всех гейтов (объединённые)
...
impl LSTMCell {
    ...
    /// Forward pass через одну LSTM ячейку
    /// Возвращает (h_new, c_new)
    pub fn forward(
        &self,
        input: &[f64],
        h_prev: &[f64],
        ...
        // Вычисляем gates = W_ih @ input + b_ih + W_hh @ h_prev + b_hh
...
/// LSTM сеть для предсказания временных рядов
...
impl LSTMPredictor {
    ...
    pub fn predict(&self, sequence: &[Vec<f64>]) -> f64 {
        ...
        // Проходим через всю последовательность
...
### 4.3.2 Transformer для финансовых данных
...
impl Attention {
    ...
    /// Вычисляет attention для одной головы
    /// Q, K, V — матрицы запросов, ключей, значений
    pub fn forward(
        &self,
        query: &[Vec<f64>],
        key: &[Vec<f64>],
        ...
        for i in 0..seq_len {
            ...
            // Apply causal mask (для трейдинга — нельзя смотреть в будущее!)
            if let Some(m) = mask {
                for j in 0..seq_len {
                    if m[i][j] {
...
/// Causal mask generator для авторегрессивных моделей
pub fn generate_causal_mask(size: usize) -> Vec<Vec<bool>> {
    let mut mask = vec![vec![false; size]; size];
    for i in 0..size {
        for j in (i + 1)..size {
            mask[i][j] = true; // Маскируем будущие позиции
        }
    }
    mask
}
...
## 4.4 Прогнозирование волатильности
...
/// Калькулятор реализованной волатильности
pub struct RealizedVolatility {
    /// Количество периодов в году (252 для дневных данных)
...
impl RealizedVolatility {
    ...
    /// Классическая realized variance (сумма квадратов returns)
    pub fn realized_variance(&self, returns: &[f64]) -> f64 {
        returns.iter().map(|r| r * r).sum()
    }

    /// Realized volatility (корень из RV)
    ...
    /// Parkinson volatility (использует high-low range)
    /// Более эффективная оценка, чем close-to-close
...
/// HAR-RV модель (Heterogeneous Autoregressive model of Realized Volatility)
/// RV_t = β₀ + β_d * RV_{t-1} + β_w * RV_week + β_m * RV_month + ε
...
impl HarRvModel {
    /// Создаёт модель с параметрами по умолчанию (типичные значения)
    pub fn default() -> Self {
        Self {
            beta_0: 0.0001,
            beta_d: 0.35,
    ...
    /// Предсказывает RV на следующий период
    ...
    /// Вычисляет компоненты RV из истории
...
## 4.5 Reinforcement Learning для трейдинга

### 4.5.1 Торговое окружение

```rust
/// Состояние торгового окружения
#[derive(Debug, Clone)]
pub struct TradingState {
    /// Нормализованный баланс (относительно начального)
    pub balance_ratio: f64,
    /// Текущая позиция (-1.0 до 1.0)
    pub position: f64,
    /// Нереализованный PnL
    pub unrealized_pnl: f64,
    /// Market features (цены, индикаторы и т.д.)
...
pub struct TradingAction {
    /// Целевая позиция от -1.0 (full short) до 1.0 (full long)
...
impl SimpleQLearner {
    ...
    /// Дискретизация состояния в bucket
    fn discretize_state(&self, state: &TradingState) -> i32 {
        // Упрощённая дискретизация по первому feature
    ...
    /// Конвертация дискретного действия в TradingAction
...
## 4.6 Deployment и Inference

### 4.6.1 Feature Pipeline для production

```rust
use std::collections::VecDeque;

/// Production-ready feature pipeline
/// Вычисляет все features в реальном времени
...
impl FeaturePipeline {
    ...
    /// Обновление pipeline новым тиком
    /// Возвращает feature vector, если накоплено достаточно данных
...
impl FeatureVector {
    ...
    /// Конвертация в плоский массив для inference
...
## 4.7 Метрики и оценка моделей

```rust
/// Метрики для оценки торговых моделей
...
**Цель:** Построить корректный backtesting pipeline
- Реализовать purged K-fold cross-validation
- Добавить embargo periods
- Моделировать transaction costs
- Вычислять метрики: Sharpe, Sortino, Calmar

### Задание 4.2: LSTM vs Transformer
**Цель:** Сравнить архитектуры на crypto данных
- Dataset: BTC-USDT 1-минутные данные, 2 года
- Target: направление движения за 5 минут
- Метрики: accuracy, precision, recall, F1, profit factor

### Задание 4.3: Volatility Forecasting Pipeline
**Цель:** Production-ready предсказание волатильности
- HAR baseline
- Deep HAR
- Интеграция с опционным pricing

### Задание 4.4: RL Trading Agent
**Цель:** Обучить и задеплоить RL агента
- Окружение с реалистичными издержками
- Сравнение PPO vs DQN
- Paper trading evaluation

### Задание 4.5: End-to-End ML System
**Цель:** Полный pipeline от данных до торговли
- Feature engineering в Rust
- Обучение модели
- Inference с low latency
- Latency benchmarks
